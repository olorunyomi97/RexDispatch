...
const Dispatchhistory = (data) => {
    ...
    useEffect(() => {
        axios.get(API_URL + '/parcel/dispatch_history/?model=business', data)
        .then(data => {
                                ...
                                <div className="p-5">
                                    {(dispatchhistory != null && dispatchhistory.length <=0) ? 
                                        <div style={{justifyContent:'center'}}> 
                                            <h5 className="pt-3">No Packages dispatched yet.</h5>
                                        </div>
                                        :
                                        <>
                                            <Table hover>
                                                ...
                                                <tbody className="table_body">
                                                {
                                                    dispatchhistory && dispatchhistory.map((history,i) => {
                                                        ...
                                                        <tr key={i} data={history}>
                                                            <td className="table_item2 p-3"><Moment format='DD-MM-YYYY'>{history.createdAt}</Moment></td>
                                                            <td className="table_item2 p-3"><Link className="dispatch_link2" to={`/business-dispatch-history-details/${history._id}`}>{history.order_number}</Link></td>
                                                            <td className="table_item2 p-3"><Link className="dispatch_link2" to={`/business-dispatch-history-details/${history._id}`}>{history.recipient_name}</Link></td>
                                                            <td className="table_item2 p-3"><Link className="dispatch_link2" to={`/business-dispatch-history-details/${history._id}`}>{history.delivery_address}</Link></td>
                                                            <td className="table_item2 p-3 credit"><Link className="transit" style={{textTransform: 'capitalize'}} to={`/business-dispatch-history-details/${history._id}`}>{history.status}</Link></td>
                                                        </tr>
                                                        )
                                                    })
                                                }
